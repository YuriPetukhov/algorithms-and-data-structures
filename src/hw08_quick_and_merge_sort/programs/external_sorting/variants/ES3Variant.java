...
import hw08_quick_and_merge_sort.libs.sorting.algorithms.external.ExternalSortES3;
...
public final class ES3Variant implements ExternalSortingVariant {
    ...
    public String displayName() {
        return "External Sort ES3 (Blocks + Merge)";
    }
    ...
    public ExternalSortSolver build(ExternalSortingParams params) {
        ...
        return job -> {
            ...
            Path presorted =
                    job.workDir().resolve("presorted.txt");

            ExternalSortES3.presortBlocks(
                    job.input(),
                    presorted,
                    params.blockSize()
            );

            Path newJobInput = presorted;

            Path f1 = job.workDir().resolve("f1.txt");
            Path f2 = job.workDir().resolve("f2.txt");
            Path temp = job.workDir().resolve("temp.txt");

            Path current = newJobInput;
