...
public final class OptimalBSTAlgorithm2 implements OptimalSearchTreeBuilder {

    @Override
    public String id() {
        return "optimal_bst_dp_optimized";
    }

    @Override
    public String displayName() {
        return "Optimal BST DP Optimized";
    }
    ...
    public OptimalSearchTree build(List<WeightedKey> keys) {
        ...
        for (int len = 2; len <= n; len++) {
            for (int left = 0; left + len - 1 < n; left++) {
                ...
                int start = root[left][right - 1];
                int end = root[left + 1][right];
