...
import hw02_dynamic_programming_and_testing.app.solver.PowerMultiplySolver;
...
public class PowerMultiplyTask implements MeasurableTask<PowerInput, Double> {

    private final Solver<PowerInput, Double> solver = new PowerMultiplySolver();

    @Override public String id() { return "power_multiply"; }

    @Override
    public String displayName() {
        return "Степень: домножение (рекурсивно) O(LogN)";
    }
    ...
    public PowerInput parse(String input) {
        ...
        if (parts.length != 2)
            throw new IllegalArgumentException("Expected: <A> <N>. Example: 1.001 1000");
        ...
        if (base <= 0.0) throw new IllegalArgumentException("base must be > 0");
